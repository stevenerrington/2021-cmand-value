...
for session = 1:length(valuedata_master.sessionN)
    ...
    postSlowingRT.ns_after_nc(session) = ...
        mean(valuedata_master.postRT(session).all.noncanc);    
    postSlowingRT.ns_after_ns(session) = ...
        mean(valuedata_master.postRT(session).all.nostop);      
    postSlowingRT.ns_after_c(session) = ...
...
%%
postSlowingRT_jasp = ...
    table(postSlowingRT.sessionN',postSlowingRT.session',postSlowingRT.monkey',...
    postSlowingRT.ns_after_nc',postSlowingRT.ns_after_ns',postSlowingRT.ns_after_c',...
    postSlowingRT.error_hi', postSlowingRT.error_lo', postSlowingRT.stop_hi', postSlowingRT.stop_lo',...
    'VariableNames',{'SessionN','Session','Monkey','NS_after_NC','NS_after_NS','NC_after_C',...
    'Error_hi','Error_lo','Stop_hi','Stop_lo'});

 writetable(postSlowingRT_jasp,...
     'D:\projectCode\project_valueStopping\data\jasp\jasp_postSlowingRT.csv',...
     'WriteRowNames',true) 
