function [bestFitParams,minDiscrepancyFn,dataplotRange,ypred] = fitWeibull_zrft(xData, yData, weights)
...
MAX_TO_1_FLAG = 0; MIN_TO_0_FLAG = 0;
% if yData(end) > .9; MAX_TO_1_FLAG = 1; minGamma = .9; end
% if yData(end) == 1; minGamma = 1; end
% if yData(1) == 0;  MIN_TO_0_FLAG = 1;    maxDelta = 0;
% end


%1) specify initial param.
alpha = 101; %alpha: time at which inhition function reaches 67% probability
beta  = 150;   %beta : slope
gamma = 0.9;   %maximum probability value
delta = 0.1;   %minimum probability value

param=[alpha beta gamma delta]; %must be in this format for ge.m

lower_bounds = [95      100       0.9      0.0];  %bounds for parameters
upper_bounds = [105     300       1.0      0.4];
...
clear ypred
dataplotRange = [95:0.1:105];
for idx = 1:length(dataplotRange)
    i = dataplotRange(idx);
    ypred(idx) = gamma - ((exp(-((i./alpha).^beta))).*(gamma-delta));
end
% 
% figure
% plot(xData,yData,'marker','o','linestyle','none');
% hold on
% plot(dataplotRange,ypred,'k')
...
ypred = gamma - ((gamma-delta)*exp(-(xData./alpha).^beta));
